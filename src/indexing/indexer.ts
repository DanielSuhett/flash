...
export class CodeIndexer {
  ...
  private extractDeclarations(content: string): Declaration[] {
    ...
    const declarationRegex = /(?:export\s+)?(?:class|interface|type|enum|function|const|var|namespace)\s+(\w+)/g;
  ...
  private findDeclarationLocation(content: string, startIndex: number): { startLine: number; endLine: number } {
  ...
  private findDependencies(content: string, location: { startLine: number; endLine: number }): string[] {
